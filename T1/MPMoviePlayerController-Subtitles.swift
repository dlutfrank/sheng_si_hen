...
public extension AVPlayerViewController {
    ...
    func deleteSubtitles() {
        if (subtitleLabel?.isDescendant(of: contentOverlayView!))! {
    ...
    fileprivate func searchSubtitles(_ time: CMTime) {
        ...
        // Adjust size
        let rect = (label.text! as NSString).boundingRect(with: CGSize(width: label.bounds.width, height: CGFloat.greatestFiniteMagnitude), options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font : label.font!], context: nil)
